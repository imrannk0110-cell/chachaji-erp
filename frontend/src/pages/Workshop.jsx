...
import { MessageCircle, Send, Edit, X, Scissors, Activity, Users, ArrowRight } from 'lucide-react';
...
const Workshop = () => {
  ...
  const [managerLedger, setManagerLedger] = useState([]);
  ...
  const handleUpdateOrder = async (e) => {
    ...
    if (isNowDelivered && balanceDue > 0) {
        ...
        return; // Pause the update, open modal instead
    }
  ...
  const executeOrderUpdate = async (paymentOverride = null) => {
      await fetch(`/api/orders/${editingOrder.id}`, {
        ...
        body: JSON.stringify({ 
            ...
            manager_id: editingOrder.manager_id,
  ...
  const handleWhatsAppTrial = (order) => {
    const phone = order.customer_phone;
    if(!phone) return alert('No phone number attached to this customer.');
    
    const text = `Hello ${order.customer_name},\n\nYour outfit at Humjoli Ethnic is Ready for Trial!\nOrder ID: ${order.id}\nBalance Due: Rs. ${order.balance_due}\n\nPlease visit the boutique at your earliest convenience. Thank you!`;
    const url = `https://wa.me/91${phone}?text=${encodeURIComponent(text)}`;
    window.open(url, '_blank');
  };

  const handleManagerClick = async (mgr) => {
    setSelectedManager(mgr);
    const res = await fetch(`/api/managers/${mgr.id}/ledger`);
    const data = await res.json();
    setManagerLedger(data);
  ...
  const handleSaveAdvance = async (e) => {
    ...
    await fetch('/api/manager_ledger', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            ...
            reference_no: `ADV-MGR-${Date.now().toString().slice(-5)}`
        })
    });
    setAdvanceAmount('');
    handleManagerClick(selectedManager); // refresh ledger
  };

  const statuses = ['Booked', 'In Workshop', 'Ready for Trial', 'Delivered'];

  // Metrics Engine
  const getManagerMetrics = (managerId) => {
      const activeOrders = orders.filter(o => o.manager_id == managerId && o.status !== 'Delivered');
  ...
  return (
    <div>
      <div className="flex justify-between items-end mb-6">
         <div>
           <h1 style={{ margin: 0 }}>Workshops Monitor</h1>
           <p className="subtext">Track real-time factory flow, metrics, and manage deadlines.</p>
         </div>
      </div>

      {/* TOP DASHBOARD: 4 Workshop Cards */}
      <div className="grid grid-cols-4 gap-6" style={{ marginBottom: '2.5rem' }}>
         {managers.slice(0, 4).map((mgr, idx) => {
             const metrics = getManagerMetrics(mgr.id);
             const colors = ['var(--accent-gold)', '#38bdf8', '#fbbf24', '#f43f5e'];
             const color = colors[idx % 4];
             return (
                 <div key={mgr.id} className="card shadow-lg hover-row" style={{ borderTop: `4px solid ${color}`, cursor: 'pointer', transition: 'all 0.2s' }} onClick={() => handleManagerClick(mgr)}>
                     <div className="flex justify-between items-center mb-4">
                        <div>
                          <p className="subtext m-0" style={{ fontSize: '0.8rem' }}>Workshop {mgr.workshop_number}</p>
                     ...
                     <div className="flex justify-between mt-6">
                        ...
                        <div className="text-center" style={{ borderLeft: '1px solid var(--border-color)', paddingLeft: '1.5rem' }}>
                            ...
                            <p className="subtext" style={{ fontSize: '0.8rem' }}>Qty/Item</p>
                        </div>
                     </div>
                 </div>
             )
         ...
         {managers.length === 0 && (
             <div className="col-span-4 p-8 text-center card subtext">No managers found. Please add them in Settings.</div>
         )}
      </div>

      {/* TIMELINE PROGRESS BOARD */}
      <div className="card" style={{ minHeight: '600px' }}>
         <h2 className="mb-6">Active Factory Timeline</h2>
            ...
            {statuses.map((statusCol, index) => (
               <div key={statusCol} className="flex flex-col" style={{ backgroundColor: 'var(--surface-color)', padding: '1rem', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
                  ...
                  <div className="flex items-center gap-3 mb-4 pb-2" style={{ borderBottom: index === statuses.length - 1 ? '2px solid var(--success)' : '2px solid var(--accent-gold)' }}>
                      <div style={{ width: '24px', height: '24px', borderRadius: '50%', backgroundColor: index === statuses.length - 1 ? 'var(--success)' : 'var(--accent-gold)', color: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '0.8rem' }}>
                         {index + 1}
                      </div>
                  ...
                  <div className="flex flex-col gap-4 flex-1">
                    {orders.filter(o => o.status === statusCol).map(order => (
                      <div key={order.id} className="p-4" style={{ backgroundColor: 'var(--bg-color)', borderRadius: '8px', border: '1px solid var(--border-color)', position: 'relative' }}>
                         ...
                         <p className="subtext mt-1 mb-3" style={{ fontSize: '0.8rem' }}>Manager: <span style={{ color: 'var(--accent-gold)' }}>{order.manager_name}</span></p>
                         ...
                         {/* Status Progress Bar Visualization */}
                         ...
                         {statusCol === 'Ready for Trial' && (
                             <button 
                                ...
                                onClick={() => handleWhatsAppTrial(order)}
                             >
                               <MessageCircle size={16} /> Notify Customer for Trial
                             </button>
                         )}
         ...
         <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
             <div className="card shadow-lg" style={{ width: '450px', position: 'relative' }}>
                 ...
                 <form onSubmit={handleUpdateOrder} className="flex flex-col gap-4">
                    ...
                    <div className="form-group mb-0">
                       <label className="form-label">Assigned Workshop Manager</label>
                       <select value={editingOrder.manager_id} onChange={e=>setEditingOrder({...editingOrder, manager_id: e.target.value})} className="w-full">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="form-group mb-0">
                           <label className="form-label">Customer Delivery</label>
          ...
          return (
             <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
                 <div className="card shadow-lg" style={{ width: '500px', position: 'relative', borderTop: '4px solid var(--success)' }}>
                     ...
                     <h2 className="mb-2">Final Delivery Settlement</h2>
                     <p className="subtext mb-6">Order {editingOrder.id} is being marked as Delivered.</p>
                     ...
                     <div className="flex justify-between items-center p-3 mb-6" style={{ backgroundColor: 'rgba(255,255,255,0.02)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                         <div>
                             <p className="subtext m-0 text-xs">Grand Total: ₹{originalOrder.grand_total}</p>
                             <p className="subtext m-0 text-xs">Advance: ₹{originalOrder.advance_paid}</p>
                         </div>
                         <div className="text-right">
                             <p className="subtext m-0 text-xs">Pending Balance</p>
                             <h3 className="m-0 text-[var(--accent-gold)]">₹{balance}</h3>
                         </div>
                     </div>

                     <p className="form-label mb-3">Remaining ₹{balance} payment received from customer?</p>
      ...
      {showManagerModal && selectedManager && (() => {
         const metrics = getManagerMetrics(selectedManager.id);
         const totalEarned = managerLedger.filter(l => l.transaction_type === 'Cr_Stitching').reduce((a, b) => a + parseFloat(b.amount), 0);
         const totalAdvance = managerLedger.filter(l => l.transaction_type === 'Dr_Advance').reduce((a, b) => a + parseFloat(b.amount), 0);
         ...
         <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
             <div className="card shadow-lg" style={{ width: '700px', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
                 <button className="btn-icon" style={{ position: 'absolute', top: '16px', right: '16px' }} onClick={() => setShowManagerModal(false)}>
                    <X size={20} />
                 </button>
                 <h2>{selectedManager.name}</h2>
                 <p className="subtext mb-6">Workshop #{selectedManager.workshop_number}</p>
                 ...
                 <div className="grid grid-cols-4 gap-4 mb-6">
                    <div className="card p-3" style={{ borderTop: '2px solid var(--border-color)', backgroundColor: 'rgba(255,255,255,0.02)' }}>
                        ...
                        <p className="subtext text-xs m-0">{metrics.itemCount} items to tailor</p>
                    </div>
                    ...
                    <div className="card p-3" style={{ borderTop: '2px solid var(--danger)', backgroundColor: 'rgba(244, 63, 94, 0.05)' }}>
                        <p className="subtext text-xs m-0">Total Advance</p>
                        <h3 className="m-0 text-sm text-danger">₹{totalAdvance.toLocaleString()}</h3>
                    </div>
                    <div className="card p-3" style={{ borderTop: `2px solid ${netBalance > 0 ? 'var(--accent-gold)' : (netBalance < 0 ? 'var(--danger)' : 'var(--success)')}`, backgroundColor: 'rgba(255,255,255,0.02)' }}>
                        <p className="subtext text-xs m-0">Net Balance</p>
                        <h3 className="m-0 text-sm" style={{ color: netBalance > 0 ? 'var(--accent-gold)' : (netBalance < 0 ? 'var(--danger)' : 'var(--success)') }}>
                        ₹{Math.abs(netBalance).toLocaleString()}
                        </h3>
                        <p className="subtext text-xs m-0" style={{ color: netBalance > 0 ? 'var(--accent-gold)' : (netBalance < 0 ? 'var(--danger)' : 'var(--success)') }}>
                            {netBalance > 0 ? 'Dues Pending' : (netBalance < 0 ? 'Advance Balance' : 'Clear')}
                        </p>
                    </div>
                 </div>

                 <div className="flex gap-2 mb-6">
                    <input type="number" placeholder="Advance Amount" className="w-full" value={advanceAmount} onChange={(e) => setAdvanceAmount(e.target.value)} />
                    <select value={paymentMode} onChange={(e) => setPaymentMode(e.target.value)}>
                       <option>Cash</option><option>Bank Transfer</option><option>UPI</option>
                    </select>
                    <button className="btn-primary" onClick={handleSaveAdvance}>Add Entry</button>
                 </div>
                          ...
                          {managerLedger.map((entry) => (
                             <tr key={entry.id} style={{ borderTop: '1px solid var(--border-color)' }}>
                                ...
                                <td className="py-2 text-xs">
                                  {entry.reference_no} 
                                  <br/><span style={{color: 'var(--text-secondary)'}}>{entry.transaction_type === 'Cr_Stitching' ? 'Stitching Pay' : 'Advance/Payment'}</span>
                                </td>
